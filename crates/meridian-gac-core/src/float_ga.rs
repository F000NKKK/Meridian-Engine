...
/// shape combinatorial explosion. Mirrored by [`crate::fixed_ga::FixedShape`]
...
/// live here instead of in whichever subsystem needed them first. The
/// axis-aligned of the two box variants — see [`Obb`] for the oriented
/// one; a cube is either with equal extents on every axis, not a separate
/// type.
#[derive(Debug, Clone, Copy, PartialEq, Default)]
pub struct Aabb {
    pub min: Vec3,
...
/// a `Projection` — see docs/graphics-design.md. No `Fixed` equivalent:
/// this feeds a GPU pipeline directly, which is `f32`-only regardless of
/// which scalar type produced the camera's pose (see
/// `crate::fixed_ga`'s module doc).
#[derive(Debug, Clone, Copy, PartialEq)]
