...
fn main() {
    ...
    let no_attenuation = AttenuationModel {
    ...
    let front = Emitter {
        frame: Motor3::translation(Vec3::new(5.0, 0.0, 0.0)),
    };
    let behind = Emitter {
        frame: Motor3::translation(Vec3::new(-5.0, 0.0, 0.0)),
    };
    ...
    for (label, emitter) in [
        ...
        let gains =
            meridian_audio_core::spatial_gains(&listener, emitter, &layout, &no_attenuation);
    ...
    println!("     amplitude-only stereo genuinely cannot tell front from back.");
    {
        ...
        check(
            "front/behind centered gain matches",
            (gain_of(&f, Channel::Left) - gain_of(&b, Channel::Left)).abs() < 1e-4,
        );
        let l = meridian_audio_core::spatial_gains(&listener, &left, &layout, &no_attenuation);
        check(
            "hard left is (near-)full Left, ~zero Right",
            gain_of(&l, Channel::Left) > 0.99 && gain_of(&l, Channel::Right) < 1e-3,
        );
    }
    ...
    for (label, emitter) in [("front (5,0,0)", &front), ("left (0,0,-5)", &left)] {
        let gains =
            meridian_audio_core::spatial_gains(&listener, emitter, &narrow, &no_attenuation);
        print_gains(label, &gains);
    }
    ...
    println!("     out there to keep blending toward, unlike headphones' ±90°.");
    {
        ...
        check(
            "hard left clamps to exactly full Left on narrow speakers too",
            (gain_of(&l, Channel::Left) - 1.0).abs() < 1e-4,
        );
    }
    ...
    for (label, emitter) in [
        ...
        let gains =
            meridian_audio_core::spatial_gains(&listener, emitter, &surround, &no_attenuation);
        print_gains(label, &gains);
    }
    ...
    println!("     front routes to Center, behind splits across the two Surrounds.");
    {
        ...
        check(
            "front -> full Center",
            (gain_of(&f, Channel::Center) - 1.0).abs() < 1e-4,
        );
        check(
            "behind has zero Center leakage",
            gain_of(&b, Channel::Center) == 0.0,
        );
        check(
            "behind splits evenly across Surround L/R",
            (gain_of(&b, Channel::SurroundLeft) - gain_of(&b, Channel::SurroundRight)).abs() < 1e-4,
        );
    }
    ...
    for (label, emitter) in [("front (5,0,0)", &front), ("left (0,0,-5)", &left)] {
        let gains =
            meridian_audio_core::spatial_gains(&listener, emitter, &surround51, &no_attenuation);
        print_gains(label, &gains);
        check(
            "LFE gain is exactly zero",
            gain_of(&gains, Channel::LowFrequency) == 0.0,
        );
    }
    ...
    check(
        "simultaneous hard-left + hard-right sums to ~equal L/R",
        (gain_of(&mixed, Channel::Left) - gain_of(&mixed, Channel::Right)).abs() < 1e-4,
    );

    println!("\n== Distance attenuation ==");
    let real_attenuation = AttenuationModel::default();
    let near = Emitter {
        frame: Motor3::translation(Vec3::new(2.0, 0.0, 0.0)),
    };
    let far = Emitter {
        frame: Motor3::translation(Vec3::new(20.0, 0.0, 0.0)),
    };
    let mono = SpeakerLayout::mono();
    let near_gain = gain_of(
        &meridian_audio_core::spatial_gains(&listener, &near, &mono, &real_attenuation),
        Channel::Center,
    );
    let far_gain = gain_of(
        &meridian_audio_core::spatial_gains(&listener, &far, &mono, &real_attenuation),
        Channel::Center,
    );
    println!("  near (d=2): gain={near_gain:.3}   far (d=20): gain={far_gain:.3}");
    check(
        "closer source is louder than farther source",
        near_gain > far_gain,
    );
