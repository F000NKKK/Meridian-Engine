...
fn main() {
    ...
    check("rotate X about Z by 90deg", quarter_turn.transform_vector(Vec3::X), Vec3::Y);
    ...
    check("rotate(X, 90deg) + translate(5,0,0)", motor.transform_point(Vec3::X), Vec3::new(5.0, 1.0, 0.0));
    ...
    check("inverse(motor).transform(motor.transform(p)) == p", round_tripped, p);

    println!("\nParent/child transform hierarchy propagation");
    let parent = Motor3::from_rotation_translation(Rotor::from_axis_angle(Vec3::Z, PI / 4.0), Vec3::new(10.0, 0.0, 0.0));
    let child = Motor3::from_rotation_translation(Rotor::from_axis_angle(Vec3::Y, PI / 6.0), Vec3::new(0.0, 2.0, 0.0));
